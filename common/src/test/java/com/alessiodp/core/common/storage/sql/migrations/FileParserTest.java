...
public class FileParserTest {
	...
	public void testQueryParser() throws IOException {
		FileParser fp = new FileParser(
				getClass().getClassLoader().getResourceAsStream("db/migrations/1__Test_queries.sql"),
				"1__Test_queries.sql"
		);
		assertEquals(
				fp.readStatement(),
				"CREATE TABLE IF NOT EXISTS ${table} ( `column1` INTEGER AUTO_INCREMENT PRIMARY KEY, `column2` VARCHAR(25) NOT NULL )"
		);
		assertEquals(
				fp.readStatement(),
				"SELECT * FROM ${table}"
		...
		assertEquals(
				fp.readStatement(),
				"ALTER TABLE ${table} ADD `column3` INTEGER DEFAULT 0 NOT NULL"
		);
